...
def start(window):
    ...
    garbledImages = utils.getGarbledImages() # returns a list of the garbled images [garbled1.png,..,garbled5.png]
    num = random.randint(0, len(garbledImages) - 1) # returns a random number between 0 and the length of the list of the garbled image's list
    filename = garbledImages[num] # get the random filename

    f = open(filepath, "r") # open the filepath
